...
EM_Feature_Table.EM_Slope(isnan(EM_Feature_Table.EM_Slope)) = 0;
EM_Feature_Table(EM_Feature_Table.y == 0,:) = [];
%EM_Feature_Table.y(EM_Feature_Table.y == 1) = 1;
EM_Feature_Table.y(EM_Feature_Table.y == 2) = 0;
...
for k = 1:K
    ...
    %accuracy(k) = sum(y_est == y_test) / length(y_test);
    % Compute feature importance for the current model
    FeatureImportanceCV(k, :) = predictorImportance(model);

    % ROC curve
    [FPR, TPR, T,~,~] = perfcurve(y_test, scores(:,2), 1); % 1 is the positive class
...
[fig_roc, opt_th_nonem, best_fpr, best_tpr] = generat_RF_EM_Part2_plots(results_kfolds.FPR, results_kfolds.TPR,results_kfolds.T,K);
...
for k = 1:K
    subplot(1,5,k)
    eval(sprintf('confusionchart(results_kfolds.y_test.k%d, results_kfolds.y_est.k%d,"Normalization","total-normalized");',k,k));
